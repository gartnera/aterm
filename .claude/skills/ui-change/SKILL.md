---
name: ui-change
description: Flow for any aterm change that affects what the user sees on screen. This includes (1) visual treatment changes — tab bar, URL preview, fonts, colors, cursor, grid layout, padding — and (2) behavioral changes whose effect shows up in rendered pixels even though no renderer code moved — new-tab cwd inheritance (different prompt path), OSC 0/1/2 title handling (different tab/window title), shell env that alters PS1, paste normalization that alters echoed bytes, mouse-mode handoff (cursor icon swaps), IME (preedit popup), font-size defaults, anything that changes what a screenshot of the same scene would show. INVOKE this skill BEFORE opening a PR for any such change, even if it feels behavioral rather than cosmetic — the litmus test is "would a screenshot of the same scene differ?". If yes, capture a before/after pair under Xvfb, push the PNGs to a `pr-assets/<topic>` orphan branch, and embed them in the PR body via raw GitHub URLs. If you only realize after merging that the skill applied, invoke it retroactively and post the before/after as a PR comment so the change has a visual record. Skip only for pure logic changes with zero on-screen effect (config parser internals, IPC plumbing that doesn't surface in the grid, refactors that don't move bytes to the renderer).
---
