---
name: ui-change
description: INVOKE before opening a PR for any aterm change where a screenshot of the same scene would look different after the change. That litmus test is the whole rule — apply it before reading the rest of this description. SKIP only for pure logic with zero rendered effect (config parser internals that no key ever reaches, IPC plumbing that doesn't surface in the grid, refactors that don't move bytes to the renderer). Adopting an alacritty config key counts as a UI change — the key exists because it changes rendering. Common triggers by key/area, beyond renderer code itself (tab bar, URL preview, fonts, colors, cursor, grid layout, padding): `[window]` (padding, opacity, decorations, `dynamic_title`, startup_mode), `[font]` (size, family, offset), `[colors]` (any), `[cursor]` (shape, blinking, thickness), `[scrolling]` (history, multiplier), `[bell]` (visual flash), `[keyboard]` bindings that produce visible effects; plus behavioral changes whose effect shows up in rendered pixels even though no renderer code moved — new-tab cwd inheritance (different prompt path), OSC 0/1/2 title handling (different tab/window title), shell env that alters PS1, paste normalization that alters echoed bytes, mouse-mode handoff (cursor icon swaps), IME (preedit popup), font-size defaults. If yes to the litmus test, capture a before/after pair under Xvfb, push the PNGs to a `pr-assets/<topic>` orphan branch, and embed them in the PR body via raw GitHub URLs. If you only realize after merging that the skill applied, invoke it retroactively and post the before/after as a PR comment so the change has a visual record.
---
